...
var array = [10,20,30];
//Create a function named 'first' that is given 'arr' as the argument and  returns the first item the given array. ARRAY AND ARR HAVE NOTHING TO DO WITH EACH OTHER! ARR IS THE ARGUMENT AND ARRAY IS THE ARRAY.
...
//Next problem RETURN ENDS A LOOP WHENEVER YOUR CODE HITS IT! ?????????
...
//Write a function called reversedLooper that is given letters as it's only argument and loops through the given array backwards alerting every item in the array starting at the end.
  ...
  //Code Here
var reversedLooper = function (let) {
...
//Have divider return an Array with the first item in the array being the evens array (all the even values from nums) and the second item in the Array being the odds array(all the odd values from nums).
  ...
  //Code Here
  var divider = function (numbs, evs, ods) {
    for (var i = evens.length -1; i >= 0; i--) {
        if (evens[i] % 2 === 0) {
            evens.splice(i,1);
        }
...
*/
  ...
  //Code Here
  var removeItem = function (list itemRemove) {
    for (var i = 0; i = list; i--) {
      return()
//listemoveItem('chips') --> ['pizza', 'hotpockets', 'MtnDew', 'corndogs'];
...
var addTen = function (numb) {
  for (var i = numb.length - 1; i >= 10; i--) {
    numb[i];
  };
}
